...
const FieldDetails = () => {
  ...
  useEffect(() => {
    async function GetData(){
      await axios.get(`http://localhost:7500/api/fields/${fieldID}`).then(async (field) => {
        console.log(field.data);
        setField(field.data);
        await axios.get(`http://localhost:7500/api/fieldOwners/${field.data.fieldOwnerId}`).then(async (fieldOwner) => {
          console.log(fieldOwner.data);
          setFieldOwner(fieldOwner.data);
          await axios.get(`http://localhost:7500/api/games/field/${fieldID}`).then((games) => {
  ...
  const deleteField = (id)=>{
    axios.delete(`http://localhost:7500/api/fields/delete/${id}`).then(
